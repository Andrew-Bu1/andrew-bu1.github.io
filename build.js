...
// Get all posts (each post is a folder with index.md)
function getAllPosts() {
  ...
  const posts = postFolders
    .map((folder) => {
      ...
      // Extract date from folder name (YYYY-MM-DD-title)
...
function generatePostPages(posts) {
  posts.forEach((post) => {
    ...
    const postDistDir = path.join(DIST_DIR, post.slug);
    ...
    // Copy all assets (images, videos, etc.) from the post folder
...
// Generate home page with list of posts
function generateHomePage(posts) {
  ...
  const homeHtml = indexTemplate.replace("{{posts}}", postsList);
